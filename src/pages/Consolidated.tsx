          ...
          {[
            ...
            <Card key={s.label} className="shadow-sm">
              <CardContent className="pt-4 pb-4">
                <div className="flex items-center gap-2 mb-1">{s.icon}<span className="text-xs text-gray-400 font-medium">{s.label}</span></div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">Click a country to view its detailed breakdown. Period: {currentMonth}</p>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {data?.rows.map((row, i) => (
                <tr key={row.country.id} className={`cursor-pointer transition-colors ${i % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-yellow-50`} onClick={() => setSelectedRow(row)}>
                  <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{row.country.name} <span className="text-xs text-gray-400">({row.country.code})</span></td>
