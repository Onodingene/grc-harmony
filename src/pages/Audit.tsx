...
const emptyAudit: Omit<AuditRecord, "id"> = { title: "", scope: "", status: "", lead: "", startDate: "" };
...
const Audit = () => {
  ...
  const openAdd = () => { setForm(emptyAudit); setOpen(true); };
  ...
  const set = (key: keyof Omit<AuditRecord, "id">, value: string) => setForm({ ...form, [key]: value });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => exportToCSV(audits, "audits")}><Download className="w-4 h-4 mr-1" /> Export CSV</Button>
          <Button onClick={openAdd}><Plus className="w-4 h-4 mr-1" /> New Audit</Button>
        </div>
      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid gap-1.5"><Label>Scope</Label><Input value={form.scope} onChange={(e) => set("scope", e.target.value)} /></div>
            ...
            <div className="grid gap-1.5"><Label>Lead</Label><Input value={form.lead} onChange={(e) => set("lead", e.target.value)} /></div>
            <div className="grid gap-1.5"><Label>Start Date</Label><Input type="date" value={form.startDate} onChange={(e) => set("startDate", e.target.value)} /></div>
          </div>
          <DialogFooter><Button onClick={save}>Save Audit</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
