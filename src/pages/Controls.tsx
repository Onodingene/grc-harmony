...
const Controls = () => {
  ...
  useEffect(() => {
    setLoading(true);
    apiFetch<Control[]>("/settings/controls")
      .then((r) => {
  ...
  const exportCSV = () => {
    const rows = filtered.map((c) => ({
      ...
      Risk: c.risk,
      Frequency: c.frequency,
      "Test Due Day": c.testDueDay ?? "",
      Status: c.status,
      Owner: c.owner?.email ?? "",
      Tester: c.tester?.email ?? "",
    }));
    const csv = [
    ...
    <div className="space-y-6">
            ...
            {loading ? (
              ...
              filtered.map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell>{c.testDueDay || "—"}</TableCell>
                  <TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter>
            <Button onClick={save}>
              {editingControl ? "Update" : "Add"} Control
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
